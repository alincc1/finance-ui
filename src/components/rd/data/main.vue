...
<template>
  <div class="fc-tpl-main" :class="mainClass" :style="mainStyle">
    ...
    <div class="fc-tpl-main--more" >
      <slot name="more"/>
    </div>
    <div class="fc-tpl-main--other" >
      <slot name="other"/>
    </div>
    <div class="fc-tpl-main--search">
      <slot name="search"/>
    </div>
...
export default {
  ...
  computed: {
    ...
    mainStyle () {
      ...
      if (!this.hasSearch) { style.height = 'calc(100% - 30px)' }
      return style
    },
    footerClass () {
      const style = {}
  ...
  methods: {
    ...
    init () {
      ...
      this.hasSearch = this.$slots.search && this.$slots.search.length
    }
  },
  created () {
    this.init()
